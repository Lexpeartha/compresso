...
void fill_byte_buffer2(Byte_buffer_static *byte_buffer, const char *string, char *output_file) {
    for (int i = 0; i < strlen(string); i++) {
        ...
        byte_buffer->index--;
        if (byte_buffer->index == -1) print_byte_buffer2(byte_buffer, output_file);
    }
}
...
int static_huffman_encode(char filename[150], char *output_file) {
    FILE *in;
    in = fopen(filename, "rb");
    ...
    while (fread(&data, sizeof(uint8_t), 1, in) == 1) {
        ...
        fill_byte_buffer2(byte_buffer, code, output_file);
    }
    ...
    print_byte_buffer2(byte_buffer, output_file);
